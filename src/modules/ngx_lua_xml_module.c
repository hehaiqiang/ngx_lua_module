...
static int ngx_lua_xml_serialize(lua_State *l);
static void ngx_lua_xml_serialize_tables(lua_State *l, axutil_env_t *env,
    axiom_node_t *parent);
...
static luaL_Reg  ngx_lua_xml_methods[] = {
    { "parse", ngx_lua_xml_parse },
    { "serialize", ngx_lua_xml_serialize },
    { NULL, NULL }
};
...
static void
ngx_lua_xml_parse_children(lua_State *l, axutil_env_t *env,
    axiom_node_t *parent, axiom_element_t *parent_elem)
{
    int                              n;
...
static int
ngx_lua_xml_serialize(lua_State *l)
{
    int                     top;
    char                   *uri, *prefix;
    axiom_node_t           *node;
    ...
    if (!lua_isnil(l, -1)) {
        ...
        ngx_lua_xml_serialize_tables(l, env, node);
    }
    ...
    axiom_node_t *parent)
{
    ...
    while (lua_next(l, -2)) {
        ...
        ngx_lua_xml_serialize_table(l, env, parent, name, index);
...
static void
ngx_lua_xml_serialize_table(lua_State *l, axutil_env_t *env,
    axiom_node_t *parent, char *name, int index)
{
    int                 top;
